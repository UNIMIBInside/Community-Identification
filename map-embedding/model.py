...
def ResNet(stack_fn,
  ...
  if (weights == 'imagenet') and (model_name in WEIGHTS_HASHES):
    if include_top:  # Giustamente ci sono pesi diversi da caricare
...
def ResNet50(include_top=True,
             ...
             input_shape=None,
             pooling=None,
             classes=1000, #inserire il numero di classi per creare
             **kwargs):
...
def Map_Embedding(include_top=True,
             ...
             pooling='avg',
             vector_space=128,
             classes=22, #inserire il numero di classi per creare
             freeze=True,
             **kwargs):
    name = 'map_embedding'
    if weights:
...
def Map_Embedding2(include_top=True,
    ...
    output_layers.append(layers.Dense(3, activation='softmax', name=name + '_output{}'.format(classes-2))(task_layers[classes-2]))
    output_layers.append(layers.Dense(3, activation='softmax', name=name + '_output{}'.format(classes-1))(task_layers[classes-1]))
